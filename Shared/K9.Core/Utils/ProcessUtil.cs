...
public static class ProcessUtil
{
    ...
    public static bool SpawnShell(string executablePath, string? arguments, string? workingDirectory, bool elevate = false)
    {
        using Process childProcess = new();
        if (elevate)
        {
	...
	{
		return Execute(executablePath, workingDirectory, arguments, input, (_, line) => log.WriteLine(line));
	}
		...
		out List<string> outputLines)
	{
		...
		int returnValue = Execute(executablePath, workingDirectory, arguments, input, (_, line) => output.Add(line));
		outputLines = output;
		return returnValue;
	}
	...
	public static int Interactive(string executablePath, string? workingDirectory, string? arguments, Action<string, StreamWriter> interactionHandler)
	{
        ...
        ProcessLogObject_StringStreamWriter logObject = new(interactionHandler, childProcess.StandardInput);
