...
public static class StringExtensions
{
	...
	/// </summary>
    const int k_AsciiNumberSign = 45;

	/// <summary>
	///     The ASCII decimal value for the decimal (.).
	/// </summary>
    const int k_AsciiNumberDecimal = 46;

	/// <summary>
	///     The ASCII decimal value for 0.
	/// </summary>
    const int k_AsciiNumberStart = 48;

	/// <summary>
	///     The ASCII decimal value for 9.
	/// </summary>
    const int k_AsciiNumberEnd = 57;

	/// <summary>
	///     The ASCII decimal value for uppercase A.
	/// </summary>
    const int k_AsciiUpperCaseStart = 65;

	/// <summary>
	///     The ASCII  decimal value for uppercase Z.
	/// </summary>
    const int k_AsciiUpperCaseEnd = 90;
	...
	///     but it is not necessary. This must be a multiple of 8 bytes.
	/// </remarks>
    static readonly byte[] k_EncryptionDefaultKey = "Awesome!"u8.ToArray();
	...
	///     but it is not necessary. This must be a multiple of 8 bytes.
	/// </remarks>
    static readonly byte[] k_EncryptionInitializationVector = "dotBunny"u8.ToArray();
	...
	public static string Decrypt(this string encryptedString, byte[]? encryptionKey = null)
	{
		DESCryptoServiceProvider desProvider = new()
        {
			...
			Key = encryptionKey is { Length: > 0 } ? encryptionKey : k_EncryptionDefaultKey,
			IV = k_EncryptionInitializationVector
		};
		using MemoryStream stream = new(Convert.FromBase64String(encryptedString));
		using CryptoStream cs = new(stream, desProvider.CreateDecryptor(), CryptoStreamMode.Read);
	...
	public static string Encrypt(this string decryptedString, byte[]? encryptionKey = null)
	{
		...
		using CryptoStream cs = new(stream, desProvider.CreateEncryptor(), CryptoStreamMode.Write);
		...
		StringComparison comparison = StringComparison.Ordinal)
	{
		...
		return splitIndex < 0 ? null : targetString[(splitIndex + splitString.Length)..];
	}
	...
	{
		int splitIndex = targetString.IndexOf(splitString, 0, comparison);
		return splitIndex < 0 ? null : targetString[..splitIndex];
	}
	...
	[SecuritySafeCritical]
	[ReliabilityContract(Consistency.WillNotCorruptState, Cer.MayFail)]
    private static unsafe int GetStableLowerCaseHashCode(this string targetString)
	{
		fixed (char* src = targetString)
		{
			int hash1 = 5381;
			...
			while ((c = s[0]) != 0)
			{
				...
				if (c is >= k_AsciiUpperCaseStart and <= k_AsciiUpperCaseEnd)
				{
					c ^= k_AsciiCaseShift;
				}
	...
	/// </summary>
	/// <remarks>
	///     This method is meant for when you do not need the value returned, merely an evaluation if
	...
	public static unsafe bool IsIntegerValue(this string targetString)
	{
		fixed (char* src = targetString)
		{
			...
			if (c != k_AsciiNumberSign && (c < k_AsciiNumberStart || c > k_AsciiNumberEnd))
			{
				return false;
			}
			...
			while ((c = s[1]) != 0)
			{
				if (c < k_AsciiNumberStart || c > k_AsciiNumberEnd)
				{
					return false;
				}
	...
	public static unsafe bool IsNumeric(this string targetString)
	{
		fixed (char* src = targetString)
		{
			...
			while ((c = s[1]) != 0)
			{
				if (c < k_AsciiNumberStart || c > k_AsciiNumberEnd)
				{
					if (c == k_AsciiNumberDecimal && !hasDecimal)
					{
						hasDecimal = true;
						s += 1;
						continue;
    ...
    public static string MarkedSubstring(this string targetString, string marker, string terminator)
    {
