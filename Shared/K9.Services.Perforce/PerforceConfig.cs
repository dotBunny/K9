...
public class PerforceConfig
{
    ...
    public static void WriteDefault(string path, string defaultPort, string characterSet, string p4Ignore)
    {
        ...
        builder.AppendLine("#");
        builder.AppendLine("# This is the username that you use to connect to the Perforce server.");
        ...
        builder.AppendLine("#");
        builder.AppendLine("# This is the hostname and port of the Perforce server, it is unlikely that you will need to change this.");
    ...
    public PerforceConfig(string path)
    {
        if (File.Exists(path))
        {
            ...
            m_Username = configUser ?? string.Empty;
    ...
    public bool IsValid()
    {
        return !string.IsNullOrEmpty(Username) && !string.IsNullOrEmpty(Port) && !string.IsNullOrEmpty(Client);
    }
}