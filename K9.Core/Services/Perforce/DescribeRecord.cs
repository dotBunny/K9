...
namespace K9.Services.Perforce
{
    public class DescribeRecord
    {
        ...
        public DescribeRecord(Dictionary<string, string> Tags)
        {
            ...
            string TimeString;
            if (!Tags.TryGetValue("time", out TimeString) || !long.TryParse(TimeString, out Time)) Time = -1;
            ...
            for (var Idx = 0;; Idx++)
            {
                var Suffix = string.Format("{0}", Idx);

                var File = new DescribeFileRecord();
                if (!Tags.TryGetValue("depotFile" + Suffix, out File.DepotFile)) break;
                ...
                if (!Tags.TryGetValue("rev" + Suffix, out RevisionString) ||
                    !int.TryParse(RevisionString, out File.Revision)) File.Revision = -1;

                string FileSizeString;
                if (!Tags.TryGetValue("fileSize" + Suffix, out FileSizeString) ||
                    !int.TryParse(FileSizeString, out File.FileSize)) File.FileSize = -1;
